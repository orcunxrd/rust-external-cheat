...
namespace driver
{
	...
	{
		memory_read = OpenFileMapping(FILE_MAP_READ, FALSE, "Global\\apovecj");
		if (memory_read == INVALID_HANDLE_VALUE || !memory_read)
			return false;

		memory_write = OpenFileMapping(FILE_MAP_WRITE, FALSE, "Global\\apovecj");
		if (memory_write == INVALID_HANDLE_VALUE || !memory_write)
			return false;

		memory_esp_write = OpenFileMapping(FILE_MAP_WRITE, FALSE, "Global\\orcunvebigsmoke");
	...
	inline uint64_t IL2CPPScan(const char* class_name)
	{
		uint64_t base = get_module_base_address("GameAssembly.dll");
		...
		for (uint64_t offset = data_size; offset > 0; offset -= 8)
		{
			...
			read(name_pointer, klass_name, sizeof(klass_name));
